...
class PageBufferManager : public BufferManager {
 public:
  ...
  ~PageBufferManager();
  ITuple *AccessTuple(ITable * tbl, SharedPtr &ref, uint64_t expected_key);
  void FinishAccessTuple(SharedPtr &ref, bool dirty);
  BasePage *AllocNewPage(OID table_id, OID page_id);
  BasePage *AccessPage(OID table_id, OID page_id);
 ...
 private:
  void FlushAndLoad(OID buf_id, PageTag &tag, bool load_from_storage);
  OID LookUpBufferId(OID bucket_id, OID table_id, OID page_id);
  OID FindBufDescToEvict();
  OID FindEmptyBufDesc();
