//
// Created by Zhihan Guo on 4/27/23.
//
...
void *BasePage::AllocDataSpace(size_t size) {
  // TODO(zhihan): assume having the latches.
  // TODO(zhihan): currently ignoring holes. in the future, we may delete record and has holes.
...
void BasePage::ShiftIfNeeded(int64_t begin, int64_t end, int64_t dest) {
  ...
  // Note(zhihan): here we assume data are of fixed sizes except for the first one (index node)!!!
