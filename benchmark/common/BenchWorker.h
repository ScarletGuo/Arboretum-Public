...
// TODO(zhihan): create ycsb worker and move the txn commit stats over there;
//  then remember to use ycsb worker instead of bench worker in ycsb workload
...
class BenchWorker : public Worker {
 public:
  ...
  static void PrintStats(uint64_t num_worker_threads) {
    ...
    auto throughput = g_stats->sum_commit_stats_.commit_cnt_ / avg_runtime;
